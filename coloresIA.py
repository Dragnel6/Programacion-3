...
def capture():
    ...
    low_yelllow = np.array([25, 192, 20], np.uint8)
    ...
    high_red2 = np.array([100, 255, 255], np.uint8)

    while True:
        comp,frame = cap.read()
        if comp == True:
            frame_hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
            yellos_mask = cv2.inRange(frame_hsv, low_yelllow, high_yellow)
            red_mask1 = cv2.inRange(frame_hsv, low_red1, high_red1)
            red_mask2 = cv2.inRange(frame_hsv, low_red2, high_red2) 
            red_mask = cv2.add(red_mask1, red_mask2)  
            
            draw(yellos_mask, [0, 255, 255], frame)
